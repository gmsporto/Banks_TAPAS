...
EQUATION("Class_Probability_Default")
// Adapted from firm equations (cf. Pedrosa & Lang, 2021). 
	v[0]=V("fs_fi_3");													// (Parameter) Requires calibration
	v[1]=VL("Class_Avg_Nominal_Income", 1);				// Replacement from operational cash flow
	v[2]=V("Class_Debt_Servicing");								// First tranche of prospective debt servicing
	v[3]=V("fs_fi_4");													// (Parameter) Requires calibration	
	v[4]=VL("Class_Debt_Rate", 1); 								// Class leverage (lagged) 
	if (v[2]!=0)
		{
		...
		{
		v[6]=V("cb_rwacpad_retail");									// (Parameter) if DS=0, PD defaults to RWAcpad for retail exposures
		}
v[7]=V("cb_rwacpad_PDfloor");
v[8]=max(v[6], v[7]);											// Input floor for PD (= 5 b.p.), according to Basel requirements
RESULT(v[8])


EQUATION("Class_Debt_Servicing")
// Adapted from firm equations(cf. Pedrosa & Lang, 2021).  
	v[0]=V("Class_Interest_Rate");
	v[1]=V("fs_lambda");												// (Parameter) Loan duration = 10 periods
	v[2]=V("Class_Demand_Loans"); 
	v[3]=(v[0]+(1/v[1]))*v[2];
RESULT(v[3])
...
// Adapted from firm equations (cf. Pedrosa & Lang, 2021).
	...
	v[1]=V("fs_mi_2");															// (Parameter) Bank's risk-aversion
	v[2]=VL("Class_Debt_Servicing", 1);
	v[3]=VL("Class_Nominal_Disposable_Income", 1); 
	if (v[3]>0)
		{	
		v[4]=v[1]*v[2]/v[3]; 													// Spread ...	
		v[5]=v[0]+v[4];																// ... added to retail interest rates
		}
	else 							  				
		{
		v[4]=v[1]*v[2];																// If income is negative, they are not used to dscount the applicable spread (banks would probably ration credit in this case, anyway).
		v[5]=v[0]*v[4];
		}
WRITE("Class_Interest_Spread", v[4]);
RESULT(v[5])


EQUATION_DUMMY("Class_Interest_Spread", "Class_Interest_Rate")


EQUATION("Class_Loan_Expected_Payback")
// adapted from firm equations. 
	v[0]=V("Class_Interest_Rate");
	v[1]=V("fs_lambda");
	v[2]=V("Class_Probability_Default");											
	v[3]=(v[1]+1)/2;	
...
//Class loans underwritten by the financial sector during current period. 

	v[0]=V("Class_Stock_Loans");
